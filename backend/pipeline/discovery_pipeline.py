...
======================
ATRT Drug Repurposing Pipeline — Main Orchestrator v1.0

Scoring architecture:
  composite_score = 0.40 × tissue_expression      (GSE70678 bulk RNA, Torchia 2015)
                  + 0.35 × depmap_essentiality     (Broad CRISPR, ATRT/rhabdoid lines)
                  + 0.20 × escape_bypass           (SMARCB1-loss resistance map)
                  + 0.05 × ppi_network             (STRING-DB proximity)

Key biology:
  EZH2 inhibitors BOOSTED (×1.40): SMARCB1 loss → EZH2 hyperactivity → synthetic lethality
  AURKA inhibitors boosted: MYCN stabilisation (especially ATRT-MYC subgroup)
  SMO/GLI inhibitors boosted in SHH subgroup (~37% of ATRT)
  BBB penalty less severe than DIPG: ATRT is NOT exclusively brainstem
...
# Sources:
#   Wilson & Roberts 2011 Nat Rev Cancer — SMARCB1 biology overview
#   Knutson 2013 PNAS — EZH2 resistance bypass via CDK4/BRD4
#   Frühwald 2020 CNS Oncology — ATRT resistance overview
...
class ATRTGenomicValidator:
    ...
    Primary data: CBTN ATRT from Cavatica (https://portal.kidsfirstdrc.org)
    Fallback: GSE70678 (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE70678)
    """
    ...
    def validate_atrt_cohort(self) -> Dict:
        ...
        if not cna_path.exists() and not rna_path.exists():
            logger.info(
                ...
                "Falling back to GSE70678. "
                "To enable CBTN: https://portal.kidsfirstdrc.org",
                self.data_dir,
            )
    ...
    def _load_gse70678_fallback(self, result: Dict) -> Dict:
        ...
        if not gse_path.exists():
            ...
            # Published prevalence priors — Johann 2016, n=150
    ...
    def calculate_smarcb1_statistics(self, genomic_stats: Dict) -> Dict:
        """Summary statistics for SMARCB1 loss. Replaces DIPG co-occurrence stats."""
        ...
        return {
            ...
            # No Fisher's test for ATRT — SMARCB1 loss is the DEFINING event
            "p_value":                None,
            "p_value_label": (
                "N/A — SMARCB1 biallelic loss is the defining event in ATRT "
                "(Hasselblatt 2011 Acta Neuropathologica PMID 20625942). "
                "No co-occurrence hypothesis is being tested."
            ),
            "statistical_note": (
...
def _apply_atrt_drug_boosts(
    ...
    """
    Apply ATRT-specific score boosts based on SMARCB1-loss biology.

    1. EZH2 inhibitors: BOOST ×1.40 (synthetic lethality — Knutson 2013)
    2. AURKA inhibitors: BOOST ×1.30 in MYC subgroup, ×1.15 otherwise (Sredni 2017)
    3. SMO/GLI inhibitors: BOOST ×1.25 in SHH subgroup (Torchia 2015)

    Applied BEFORE BBB penalty.
    """
    ...
    logger.info(
        "ATRT drug boosts: EZH2×%d | AURKA×%d | SMO×%d",
...
class ProductionPipeline:
    ...
    Steps:
      1. Fetch drugs from OpenTargets API (EFO IDs for ATRT/rhabdoid)
      2. Load ATRT genomic data (CBTN or GSE70678 fallback)
      3. Score tissue expression via GSE70678 bulk RNA
    ...
    async def analyze_disease(
        ...
        """Entry point for main.py /analyze endpoint."""
        result = await self.run(
            disease_name=disease_name,
            top_k=max_results,
        )
        candidates = result.get("top_candidates", [])
        # Normalise field names for the main.py filter
        for c in candidates:
    ...
    async def run(
        ...
        logger.info(
            "ATRT genomics: %s | %d upregulated genes | SMARCB1 loss: %d samples | "
            "escape bypass: %s",
        ...
        # Step 6: ATRT drug boosts (EZH2, AURKA, SMO)
        candidates = _apply_atrt_drug_boosts(candidates, subgroup=subgroup)

        # Step 7: BBB filter + location-aware penalty
